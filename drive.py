...
class CameraManager(object):
    def __init__(self, parent_actor, hud):
        ...
        self._camera_transforms = [
            carla.Transform(carla.Location(x=1.7, z=1.7), carla.Rotation(pitch=-15)),
        ...
        for item in self._sensors:
            ...
            if item[2] == 'Camera RGB':
                bp.set_attribute('image_size_x', str(200))
                bp.set_attribute('image_size_y', str(66))
            item.append(bp)
        self._index = None
    ...
    def _parse_image(weak_self, image):
        ...
        if self._sensors[self._index][0].startswith('sensor.lidar'):
            ...
            img = array
        if self._recording:
            for col in self._hud.measurements['collision']:
                if col > 0:
                    self.toggle_recording()
                    print('Bye!')
            if self._recording:
                recording.save_to_csv(frame_number, image, self._hud.measurements, high_level_command)
                frame_number += 1
...
def game_loop(args):
    ...
    try:
        ...
        if args.agent == "Roaming":
            ...
            spawn_point = world.world.get_map().get_spawn_points()[2]
            agent.set_destination((spawn_point.location.x,
                                   spawn_point.location.y,
                                   spawn_point.location.z))
        else:
            agent = ImitationLearning(world.vehicle)
        ...
        while True:
            ...
            pygame.display.flip()
            if args.agent == "Roaming":
                control, high_level_command = agent.run_step(debug=False)
            elif args.agent == "Basic":
                control, high_level_command = agent.run_step(debug=False)
            else:
                agent.set_image(img)
...
def main():
    ...
    argparser.add_argument(
        ...
        default='400x500',
        help='window resolution (default: 400x500)')

    argparser.add_argument("-a", "--agent", type=str,
                           choices=["Roaming", "Imitation", "Basic"],
